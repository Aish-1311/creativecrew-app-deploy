...
const Manager = () => {
  ...
  const onClickingAddEmployeeButton = async () => {
    if (newFirstName !== "" && newLastName !== "") {
      try {
        ...
        const sortedEmployeeList = employeeList.sort((x, y) =>
  ...
  const getManagerDetails = async (id) => {
    try {
      ...
      if (token) {
        try {
          ...
          const response3 = await axios.get(
            `${baseurl}/getManagerTeamsFromTable`,
            {
              params: {
                // Using params to send query parameters
                bu: decode.bu,
          ...
          setSelectedTeam(
            response3.data.find((team) => team.team === "Default").id
          );
          setDefaultId(teamId);
        } catch (err) {
          console.error("Error fetching data:", err);
      ...
      const filteredManager = response1.data.find(
        (manager) => manager.team_id === teamId
      );
      ...
      if (response2.data.length > 0) {
        const filteredEmployeeList = response2.data.filter(
          (item) => item.team_id === teamId
        );
        if (selectedEmployee === "") {
          setSelectedEmployee({
            ...filteredEmployeeList[0],
            name:
              filteredEmployeeList[0].first_name +
              " " +
              filteredEmployeeList[0].last_name,
            seats_array: [
              ...new Set(Object.values(filteredEmployeeList[0].seat_data)),
            ],
          });
  ...
  const handleTeamChange = (event) => {
    ...
    if (event.target.value !== "addNewTeam") {
      setManager(
        managersList.find((item) => item.team_id === event.target.value)
      );
    }
    const filteredList = employees.filter(
      (manager) => manager.team_id === event.target.value
    );
  ...
  const createTeam = async () => {
    try {
      ...
      if (response.data !== "" && response.data.data.length > 0) {
        ...
        setOpenTeamSnackbar(true);
        setSelectedTeam(
          response.data.data.filter((team) => team.team === newTeam)[0].id
        );
        setNewTeam("");
    ...
    if (selectedEmployee) {
      try {
        ...
        const employeeObject = {
          ...
          seats_array: [...new Set(Object.values(updatedEmployee.seat_data))],
        };
        const updateEmployeesList = employees.filter(
  ...
  const onCancellingAddEmployee = () => {
    ...
    setIsAddingEmployee(false);
    setSelectedEmployee(
      employees.filter((item) => item.team_id === defaultId)[0]
    );
    setSeatData(
      employees.filter((item) => item.team_id === defaultId)[0].seat_data
    );
    setNewFirstName(""); // Clear the input fields
    setNewLastName("");
    setNewFirstNameError(""); // Clear any previous error messages
    ...
    for (let i = 1; i <= manager.total; i++) {
      seats.push(
        <Seat
          ...
          employeesList={employees.filter(
            (employee) => employee.team_id === selectedTeam
          )}
  ...
  const employeesAllocatedOnSelectedDay = () => {
    ...
    return employees.filter(
      (employee) =>
        employee.team_id === selectedTeam &&
        typeof employee.seat_data[selectedDay] === "number"
    ).length;
  };
  ...
  const editTeamName = async () => {
    try {
      ...
      if (response.data !== "" && response.data.data.length > 0) {
        ...
        setOpenTeamSnackbar(true);
        setSelectedTeam(
          response.data.data.filter((team) => team.team === editTeam.team)[0].id
        );
        setEditTeam(null);
      }
    } catch (err) {
    ...
    if (selectedNewEmployee.team_id !== defaultId) {
      ...
      try {
        ...
        if (response.data !== "" && response.data.data.length > 0) {
          ...
          setSnackBarText(response.data.msg);
          setOpenTeamSnackbar(true);
          const employeesList = response.data.data.map((item) => ({
            ...item,
            name: item.first_name + " " + item.last_name,
            seats_array: [...new Set(Object.values(item.seat_data))],
          }));
          ...
          setSelectedEmployee(
            employeesList.filter(
              (employee) => employee.name === selectedNewEmployee.name
            )[0]
          );
          setSeatData(
            employeesList.filter(
  ...
  return (
    ...
    >
        ...
        >
          <Table>
            ...
            <TableBody>
              <TableRow>
                <TableCell>
                  Seats Assigned To Employees on {`${selectedDay}`}
                </TableCell>
      ...
      >
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              renderValue={(selected) => {
                ...
                return selectedTeamObj.team !== "Default"
            ...
            >
              <MenuItem
                key={"addNewTeam"}
                value={"addNewTeam"}
                sx={{ color: "#817EF2", fontStyle: "italic" }}
              >
                Add New Team
              </MenuItem>
              {teams.map((team) => (
        ...
        {!isAddingNewTeam && !isSearching && (
          ...
          >
            ...
            <Select
              ...
              renderValue={(selected) => {
                // Find selected team name and display ONLY the name
                const selectedTeamObj = employees.find(
            ...
            >
              ...
              {employees.map((manager) => {
                if (manager.team_id === selectedTeam)
                  return (
                    <MenuItem
                      key={manager.id}
                      value={manager.id}
                    ...
                    >
                      ...
                      {selectedTeam !== defaultId && (
                        <Box>
                          <IconButton
                            onClick={() =>
                              handleClickDeleteEmployeeFromTeam(manager.id)
                            }
                          >
                            <DeleteIcon />
                          </IconButton>
        ...
        >
          <DialogTitle style={{ color: "red" }}>Confirm Removal</DialogTitle>
          <DialogContent>
            <DialogContentText>
              Are you sure you want to remove employee{" "}
              <strong>{selectedEmployee?.name}</strong> from the list?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            ...
            <Button onClick={handleConfirmRemoveEmployee} color="error">
              Remove
            </Button>
          </DialogActions>
        </Dialog>
        ...
        {teams.length > 0 &&
          ...
          selectedEmployee !== "" &&
          selectedTeam !== "" && (
            <Dialog open={openEmployeeTeamDelete}>
              <DialogTitle style={{ color: "red" }}>Caution!</DialogTitle>
              <DialogContent>
                <DialogContentText>
                  Do you want to delete{" "}
                  {
                    employees.find((item) => item.id === selectedEmployee.id)
                      .name
                  }{" "}
                  from {teams.find((item) => item.id === selectedTeam).team}?"
                </DialogContentText>
              </DialogContent>
              <DialogActions>
                <Button onClick={handleClose} color="primary">
                ...
                <Button
                  onClick={() => handleDeleteEmployeeFromTeam(selectedEmployee)}
                  color="warning"
                >
                  Proceed
                </Button>
      ...
      >
        {days.map((day) => (
          <Button
            key={day.id}
            variant="contained"
            disabled={isAddingNewTeam ? true : false}
        ...
        <Paper elevation={0} style={{ padding: "20px", marginTop: "20px" }}>
          ...
          >
            ...
            <Button
              variant="contained"
              color="primary"
              onClick={onCancellingAddEmployee}
            >
              Cancel
            </Button>
          </Box>
